...
void handleRoot() {
  ...
  for (auto& log : fileLogs) {
    ...
    html += "<td>" + String(log.maxLatency) + "</td>";
    html += "<td>" + String(log.minLatency) + "</td>";
    html += "<td>" + String(log.successPackets) + "</td>";
    html += "<td>" + String(log.errorPackets) + "</td>";
    html += "</tr>";
  }
...
void handleFileUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    if (file) {
      ...
      unsigned long latency = currentTime - (currentFileInfo.packetTimes.empty() ? currentFileInfo.startTime : currentFileInfo.packetTimes.back());
      currentFileInfo.packetTimes.push_back(currentTime);
      currentFileInfo.totalLatency += latency;
      if (latency > currentFileInfo.maxLatency) currentFileInfo.maxLatency = latency;
      if (latency < currentFileInfo.minLatency) currentFileInfo.minLatency = latency;

      bool success = random(0, 2); // Simulate packet success or failure
      if (success) {
        currentFileInfo.successPackets++;
        Serial.print("Packet Latency: ");
        Serial.print(latency);
        Serial.println(" ms, Success: Yes");
      } else {
        currentFileInfo.errorPackets++;
        Serial.print("Packet Latency: ");
        Serial.print(latency);
        Serial.println(" ms, Success: No");
      }
